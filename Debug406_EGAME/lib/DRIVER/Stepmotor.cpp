...
// void SingleMotors_Init(MotorState_t *Motor,MotorIndex_t Index, uint8_t addr, bool isRun, Dir_t dir, uint8_t acc, uint16_t targetSpeed)
// {
//   Motor->addr =  addr;
//   Motor->isRun = isRun;
//   Motor->dir = dir;
//   Motor->targerSpeed = targetSpeed;
//   Motor->acc = acc;
//   Motor->currSpeed = 0.0f;
//   Motor->currAngle = 0.0f;
...
// {
//   SingleMotors_Init(&Motor, Motor_1, 1, false, CW, 0, 1000);
// }
...
void Emm_V5_Receive_Data(uint8_t *rxCmd, uint8_t *rxCount)
{
  ...
  while(1)
  {
    ...
    else                                  // 串口有没有数据
    {
      ...
      //Serial.println("None");
      if((int)(cTime - lTime) > 5)      // 100毫秒内串口没有数据进来，就判定一帧数据接收结束
      {
        *rxCount = i;                     // 数据长度
